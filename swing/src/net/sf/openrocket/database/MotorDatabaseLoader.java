...
import net.sf.openrocket.file.DirectoryWatcher;
...
public class MotorDatabaseLoader extends AsynchronousDatabaseLoader {
    ...
    private void loadUserDefinedMotors() {
        ...
        if (directories.size() > 0) {
            setWatcher(loader, directories);
        }
        log.info("Ending reading user-defined motors, motorCount=" + motorCount);
    }

    private void setWatcher(GeneralMotorLoader loader, List<Path> paths) {
        DirectoryWatcher.Builder builder = new DirectoryWatcher.Builder();
        builder.addDirectories(paths);
        builder.setFilter(path -> {
            var nPath = path.toString().toLowerCase(Locale.ENGLISH);
            for (String ext : loader.getSupportedExtensions()) {
                if (nPath.endsWith(ext))
                    return true;
            }
            return false;
        });

        var watcher = builder.build((event, path) -> {
            //If it was a delete, dont call loader
            if ((event != DirectoryWatcher.Event.ENTRY_DELETE)) {
                loadFile(loader, path.toFile());
            }
        });
